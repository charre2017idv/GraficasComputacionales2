...
struct PS_INPUT
{
	...
	float4 Color : COLOR0;
#else
	//float3 wsViewDir : TEXCOORD0;
	float3 wsNormal : TEXCOORD1;  // Vertex normal (World Space)
	float3 wsPos    : TEXCOORD2;  // Vertex position (World Space)
	//float3 wsLigthDir : TEXCOORD3;
	//float3 wsTangent   : TEXCOORD3;  // Vertex tangent (World Space)
#endif
};
...
PS_INPUT VS(VS_INPUT input)
{
	...
	float3 NormalWS = normalize(mul(float4(input.Normal.xyz, 0.0), World).xyz);  // Normal World Space
#ifdef VER_LIGTH
	// Lambert light calc
	float3 LightDirWs = normalize(-LightDir);
	float NdL = dot(LightDirWs, NormalWS);
	//output.Color = Ndl.xxx;
	...
	//output.Color = float4(NormalWS.xyz, 1);//float4(Ndl.xxx * DifuseColor.xyz, 1);
	...
	//output.wsViewDir = wsViewDir.xyz;
	//output.wsLigthDir = LightDirWs.xyz;
#endif
	return output;
}
...
float4 PS(PS_INPUT input) : SV_Target
{
	...
	//return DifuseColor;
	//return txDiffuse.Sample(samLinear, input.Tex) * float4((wsNdL.xxx ) + (SpecularFactor.xxx),1.0) ;
	return txDiffuse.Sample(samLinear, input.Tex) * float4((wsNdL.xxx * DifuseColor.xyz) + (SpecularFactor.xxx * SpecularColor.xyz),1.0) ;
	//return txDiffuse.Sample(samLinear, input.Tex)  * float4((wsNdL.xxx) + (SpecularColor.xyz), 1.0);
