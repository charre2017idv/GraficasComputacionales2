...
void CShader::setUniform3f(const string & name, float x, float y, float z)
{
	GLCall(glUniform3f(GetUniformLocatiiion(name), x, y, z));
}
...
void CShader::setUniform4fv(const string & name, int amount, float x[])
{
	GLCall(glUniform4fv(GetUniformLocatiiion(name), amount, &x[0]));
}
